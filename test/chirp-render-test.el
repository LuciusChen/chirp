  ...
  (let ((tweet (chirp-test--sample-quoted-tweet-with-media))
    ...
    (with-temp-buffer
      ...
      (let ((quoted-prefix-lines 0)
        ...
        (setq display-pos (point-min))
        (while (and (< display-pos (point-max))
                    (not (eq (car-safe (car-safe (get-text-property display-pos 'display)))
                             'slice)))
          (setq display-pos
                (or (next-single-property-change
                     (1+ display-pos) 'display nil (point-max))
                    (point-max))))
        (when (>= display-pos (point-max))
          (setq display-pos nil))
        (should (>= quoted-prefix-lines 4))
        (should (>= zero-spacing-newlines 2))
        (should display-pos)
        (goto-char display-pos)
        (setq prefix-display-pos (line-beginning-position))
